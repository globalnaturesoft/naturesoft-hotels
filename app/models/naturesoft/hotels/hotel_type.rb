module Naturesoft::Hotels
  class HotelType < ApplicationRecord
    ...
    def self.sort_by
      [
        ["Created At","naturesoft_hotels_hotel_types.created_at"],
        ["Name","naturesoft_hotels_hotel_types.name"],
      ]
    end
    ...
    def self.search(params)
      ...
      if params[:keyword].present?
        params[:keyword].split(" ").each do |k|
          records = records.where("LOWER(CONCAT(naturesoft_hotels_hotel_types.name)) LIKE ?", "%#{k.strip.downcase}%") if k.strip.present?
        end
      end
      
      # for sorting
      sort_by = params[:sort_by].present? ? params[:sort_by] : "naturesoft_hotels_hotel_types.name"
