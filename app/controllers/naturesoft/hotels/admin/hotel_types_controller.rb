...
module Naturesoft
  module Hotels
    module Admin
      class HotelTypesController < Naturesoft::Admin::AdminController
        before_action :set_hotel_type, only: [:show, :edit, :update, :destroy, :enable, :disable]
        before_action :default_breadcrumb
        
        # add top breadcrumb
        def default_breadcrumb
          add_breadcrumb "Hotel", naturesoft_hotels.admin_hotel_types_path
          add_breadcrumb "Hotel Types", naturesoft_hotels.admin_hotel_types_path
        end
        ...
        def create
          ...
          if @hotel_type.save
            redirect_to naturesoft_hotels.edit_admin_hotel_type_path(@hotel_type.id), notice: 'Hotel type was successfully created.'
          else
            render :new
          end
        end
        ...
        def update
          if @hotel_type.update(hotel_type_params)
            redirect_to naturesoft_hotels.edit_admin_hotel_type_path(@hotel_type.id), notice: 'Hotel type was successfully updated.'
          else
            render :edit
          end
        end
