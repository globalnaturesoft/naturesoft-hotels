module Naturesoft
  module Hotels
    module Backend
      class ServicesController < Naturesoft::Backend::BackendController
        before_action :set_service, only: [:show, :edit, :update, :destroy, :enable, :disable]
        before_action :default_breadcrumb
        
        # add top breadcrumb
        def default_breadcrumb
          add_breadcrumb "Hotel", naturesoft_hotels.backend_services_path
          add_breadcrumb "Service", naturesoft_hotels.backend_services_path
        end
        ...
        def create
          ...
          if @service.save
            redirect_to naturesoft_hotels.edit_backend_service_path(@service.id), notice: 'Service was successfully created.'
          else
            render :new
          end
        end
        ...
        def update
          if @service.update(service_params)
            redirect_to naturesoft_hotels.edit_backend_service_path(@service.id), notice: 'Service was successfully updated.'
          else
            render :edit
          end
        end
