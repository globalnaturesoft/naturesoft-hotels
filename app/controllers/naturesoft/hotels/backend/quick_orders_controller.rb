module Naturesoft
  module Hotels
    module Backend
      class QuickOrdersController < Naturesoft::Backend::BackendController
        before_action :set_quick_order, only: [:show, :edit, :update, :destroy, :enable, :disable]
        before_action :default_breadcrumb
        
        # add top breadcrumb
        def default_breadcrumb
          add_breadcrumb "Hotel", naturesoft_hotels.backend_quick_orders_path
          add_breadcrumb "Quick booking orders", naturesoft_hotels.backend_quick_orders_path
        end
        ...
        def create
          ...
          if @quick_order.save
            redirect_to naturesoft_hotels.edit_backend_quick_order_path(@quick_order.id), notice: 'Quick order was successfully created.'
          else
            render :new
          end
        end
        ...
        def update
          if @quick_order.update(quick_order_params)
            redirect_to naturesoft_hotels.edit_backend_quick_order_path(@quick_order.id), notice: 'Quick order was successfully updated.'
          else
            render :edit
          end
        end
