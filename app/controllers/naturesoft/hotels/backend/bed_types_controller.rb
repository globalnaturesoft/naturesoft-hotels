module Naturesoft
  module Hotels
    module Backend
      class BedTypesController < Naturesoft::Backend::BackendController
        before_action :set_bed_type, only: [:show, :edit, :update, :enable, :disable, :destroy]
        before_action :default_breadcrumb
        
        # add top breadcrumb
        def default_breadcrumb
          add_breadcrumb "Hotel", naturesoft_hotels.backend_bed_types_path
          add_breadcrumb "Bed Types", naturesoft_hotels.backend_bed_types_path
        end
        ...
        def create
          ...
          if @bed_type.save
            redirect_to naturesoft_hotels.edit_backend_bed_type_path(@bed_type.id), notice: 'Bed type was successfully created.'
          else
            render :new
          end
        end
        ...
        def update
          if @bed_type.update(bed_type_params)
            redirect_to naturesoft_hotels.edit_backend_bed_type_path(@bed_type.id), notice: 'Bed type was successfully updated.'
          else
            render :edit
          end
        end
