module Naturesoft
  module Hotels
    module Backend
      class EventCategoriesController < Naturesoft::Backend::BackendController
        before_action :set_event_category, only: [:show, :edit, :update, :destroy, :enable, :disable]
        before_action :default_breadcrumb
        
        # add top breadcrumb
        def default_breadcrumb
          add_breadcrumb "Event Category", naturesoft_hotels.backend_event_categories_path
        end
        ...
        def create
          ...
          if @event_category.save
            redirect_to naturesoft_hotels.edit_backend_event_category_path(@event_category.id), notice: 'Event category was successfully created.'
          else
            render :new
          end
        end
        ...
        def update
          if @event_category.update(event_category_params)
            redirect_to naturesoft_hotels.edit_backend_event_category_path(@event_category.id), notice: 'Event category was successfully updated.'
          else
            render :edit
          end
        end
