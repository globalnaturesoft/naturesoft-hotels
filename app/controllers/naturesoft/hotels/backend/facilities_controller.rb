module Naturesoft
  module Hotels
    module Backend
      class FacilitiesController < Naturesoft::Backend::BackendController
        before_action :set_facility, only: [:show, :edit, :update, :enable, :disable, :destroy]
        before_action :default_breadcrumb
        
        # add top breadcrumb
        def default_breadcrumb
          add_breadcrumb "Hotel", naturesoft_hotels.backend_facilities_path
          add_breadcrumb "Facilities", naturesoft_hotels.backend_facilities_path
        end
        ...
        def create
          ...
          if @facility.save
            redirect_to naturesoft_hotels.edit_backend_facility_path(@facility.id), notice: 'Facility was successfully created.'
          else
            render :new
          end
        end
        ...
        def update
          if @facility.update(facility_params)
            redirect_to naturesoft_hotels.edit_backend_facility_path(@facility.id), notice: 'Facility was successfully updated.'
          else
            render :edit
          end
        end
